...
The placebo in row R5 dates coverage three years before the first score and uses only years before the actual score. It yields an effect of {{mtb_R5}} on log MTB (*p* {{mtb_R5_p_txt}}) and of {{mcap_R5}} on log market capitalization (*p* {{mcap_R5_p_txt}}). The placebo fails, and the failure restates the pre-coverage growth shown in Fig. 1 in a different form rather than adding independent evidence. Row R6 reports the static two-way fixed effects regression of Eq. (11). It attributes an increase of {{mtb_R6}} in log MTB (*p* {{mtb_R6_p_txt}}) and of {{mcap_R6}} in log market capitalization to coverage. Because it compares all post-coverage years with all earlier years, it counts part of the pre-coverage growth as an effect of the rating. The gap between rows R6 and the baseline is an example of the biases of static two-way fixed effects regressions under staggered timing described by Goodman-Bacon (2021) and Baker, Larcker, and Wang (2022).
...
note:=ATT is the average treatment effect on the treated from Eq. (6) with bootstrap standard errors and percentile 95% confidence intervals; p-values for the three secondary outcomes are Holm-adjusted. MDE is the minimum detectable effect at 80% power, and pre-trend p is the joint test of Eq. (8). Log outcomes are in log points and book leverage is a ratio.
source:=Authors' calculations from LSEG ESG scores and Compustat Global.
